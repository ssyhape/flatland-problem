...
def A_star_normal_hc(start: tuple,start_direction: int,goal: tuple,rail: GridTransitionMap,existing_paths: list, vertex_constraints: list,edge_constraints:list,agent_id , start_stay:int):
    ...
    if start_stay > 0:
        for i in range(start_stay):
            tmp = A_star_node(start,start_direction,i+1)
...
def get_new_constraints2(path_all):
    """
    :return:  Get first confliction
    """
    new_vertex_constraints = [[] for _ in range(len(path_all))]
...
def priority_certify(item):
    return item.deadline - h_func(item.target,item.initial_position)

def do_sort(agents: List[EnvAgent]):
    sorted_list = sorted(agents,key=priority_certify)
    sorted_indexes = [idx for idx, val in sorted(enumerate(agents), key=lambda x: priority_certify(x[1]))]

    return sorted_indexes,sorted_list

def sort_list_by_indexes(original_list,sorted_indexes):
    sorted_list = [[] for _ in range(len(sorted_indexes))]
    for path,index in zip(original_list,sorted_indexes):
        sorted_list[index] = path
    return sorted_list
...
def get_path(agents: List[EnvAgent],rail: GridTransitionMap, max_timestep: int):
    ...
    sorted_index,sorted_list = do_sort(agents)
    oral_agents = agents
    agents = sorted_list
    ...
    for agent_id in range(0,len(agents)):
        ...
        path = A_star_normal_hc(loc,direction,goal,rail,root.solutions,root.vertexconstraints,root.edgeconstraints,agent_id,0)
    ...
    while openlist:
        ...
        new_vertex_constraints,new_edge_constraints = get_new_constraints2(P.solutions)

        if not (any(new_vertex_constraints) or any(new_edge_constraints)):
            solutions = sort_list_by_indexes(P.solutions,sorted_index)
            return solutions
        ...
        for agent_id in agents_conflict:
            ...
            #Only modified the agent_id
            agents_need_modified = [agent_id]
            ...
            direction = agents[agent_id].initial_direction
            goal = agents[agent_id].target
            path = A_star_normal_hc(loc, direction, goal, rail, A.solutions[:agent_id],
                                    A.vertexconstraints[agent_id], A.edgeconstraints[agent_id],
                                    agent_id, 1)
            A.solutions[agent_id] = path
            for id in range(agent_id,len(agents)):
                loc = agents[id].initial_position
                direction = agents[id].initial_direction
                goal = agents[id].target
                path = A_star_normal_hc(loc, direction, goal, rail, A.solutions[:id] ,
                                    A.vertexconstraints[id], A.edgeconstraints[id],
                                    id, 1)
                A.solutions[id] = path
            """
...
def get_replan_paths(agents: List[EnvAgent],rail: GridTransitionMap, max_timestep: int):
    ...
    for agent_id in range(0,len(agents)):
        ...
        """
        if agents[agent_id].malfunction_data["malfunction"]>0:
            for i in range(agents[agent_id].malfunction_data["malfunction"]+1):
                path = A_star_normal_hc(loc,direction,goal,rail,root.solutions,root.vertexconstraints,root.edgeconstraints,agent_id,agents[agent_id].malfunction_data["malfunction"]-i)
                if path!=[]:
                    break
        else:
        """
    ...
    for id, path in enumerate(root.solutions):
        if path == []:
            loc = agents[id].position
            direction = agents[id].direction
            goal = agents[id].target
            path = []
            """
            if agents[id].malfunction_data["malfunction"] > 0:
                for i in range(agents[id].malfunction_data["malfunction"]+1):
                    path = A_star_normal_hc(loc, direction, goal, rail, [], root.vertexconstraints,
                                            root.edgeconstraints, id,
                                            agents[id].malfunction_data["malfunction"] - i)
                    if path != []:
                        break
            else:
            """
    ...
    while openlist:
        ...
        for agent_id in agents_conflict:
            ...
            for id in agents_need_modified:
                ...
                if agents[id].malfunction_data["malfunction"] > 0:
                    for i in range(agents[id].malfunction_data["malfunction"]+1):
                        path = A_star_normal_hc(loc, direction, goal, rail, A.solutions[:id]+A.solutions[id+1:], A.vertexconstraints,
                                                A.edgeconstraints, id,
                                                agents[id].malfunction_data["malfunction"] - i)
                        if path != []:
                            break
                ...
                """
                path =A_star_normal_hc(loc, direction, goal, rail, A.solutions[:id]+A.solutions[id+1:], A.vertexconstraints[id],
                                                A.edgeconstraints[id], id,0)
                A.solutions[id] = path
            #Make sure generate before is good.
            for id,path in enumerate(A.solutions):
                if path == []:
                    loc = agents[id].position
                    direction = agents[id].direction
                    goal = agents[id].target
                    ...
                    if agents[id].malfunction_data["malfunction"] > 0:
                        for i in range(agents[id].malfunction_data["malfunction"]+1):
                            path = A_star_normal_hc(loc, direction, goal, rail, [], A.vertexconstraints,
                                                    A.edgeconstraints, id,
                                                    agents[id].malfunction_data["malfunction"] - i)
                            if path != []:
                                break
                    ...
                    """
                    path = A_star_normal_hc(loc, direction, goal, rail, [], A.vertexconstraints[id],
                                                A.edgeconstraints[id], id,0)
                    A.solutions[id] =path
    ...
    return [[] for _ in range(len(agents))]
...
def replan(agents: List[EnvAgent],rail: GridTransitionMap,  current_timestep: int, existing_paths: List[Tuple], max_timestep:int, new_malfunction_agents: List[int], failed_agents: List[int]):
    ...
    for agent_id in range(len(agents)):
        if  agents[agent_id].position!=None :
    ...
    for i,id in enumerate(unfinished_id):
        existing_paths[id] = paths_before_current_time[id] + path_after_timespace[i]
